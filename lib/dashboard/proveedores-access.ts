...
import { isProveedoresModule } from '@/lib/dashboard/proveedores-module'

export async function userCanAccessProveedores(): Promise<boolean> {
  ...
  return ((accessRows ?? []) as unknown as AccessRow[]).some(row => {
    ...
    return isProveedoresModule(mod.slug, mod.name)
  })
}
