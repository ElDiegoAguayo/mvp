-- Inspectores: Asistencia técnica + Estados fenológicos + Estimación de cosecha (conteo)
...
BEGIN
  ...
  WHERE m.slug IN ('asistencia-tecnica', 'estados-fenologicos', 'estimacion-cosecha')
    AND m.id = NEW.module_id;

  IF allowed_count = 0 THEN
    RAISE EXCEPTION 'Los inspectores de campo solo pueden tener Asistencia técnica, Estados fenológicos y Estimación de cosecha (conteo)';
  END IF;
...
BEGIN
  ...
  SELECT
    ...
    true,
    CASE m.slug
      WHEN 'asistencia-tecnica' THEN 0
      WHEN 'estados-fenologicos' THEN 1
      WHEN 'estimacion-cosecha' THEN 2
    END
  FROM public.modules m
  WHERE m.slug IN ('asistencia-tecnica', 'estados-fenologicos', 'estimacion-cosecha')
...
WHERE uma.user_id = p.id
  ...
  AND m.slug NOT IN ('asistencia-tecnica', 'estados-fenologicos', 'estimacion-cosecha');
...
WHERE p.is_tech_inspector = true
  AND m.slug IN ('asistencia-tecnica', 'estados-fenologicos', 'estimacion-cosecha')
