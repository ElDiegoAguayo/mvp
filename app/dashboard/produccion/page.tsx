...
import { calcularCapacidadPorReceta } from '@/app/actions/produccion'
...
export default async function ProduccionPage() {
  ...
  const { ok, data, message } = await calcularCapacidadPorReceta(ownerId)
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="rounded-xl border border-red-500/40 bg-red-950/20 p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-red-300/70 mt-0.5">
              {critCount > 0 && `${critCount} crítico${critCount > 1 ? 's' : ''} (< 10 pallets) · `}
              {bajCount > 0 && `${bajCount} bajo${bajCount > 1 ? 's' : ''} (< 50 pallets)`}
            </p>
          </div>
        </div>
      )}
      ...
      {!ok ? (
        ...
        <WidgetAlertasProduccion data={data} />
      )}

      {/* Footer note */}
      {ok && data.length > 0 && (
        <p className="text-xs text-muted-foreground/50 text-center">
          Umbrales: Crítico &lt; 10 pallets · Bajo &lt; 50 pallets · OK ≥ 50 pallets
        </p>
      )}
    </div>
  )
